...
public class BenchmarkTypeSuite
{
    ...
    {
        long count = 0;
        await foreach (var row in _native.QueryStreamAsync($"SELECT v FROM {table}{Settings}"))
        {
    ...
    {
        await foreach (var row in _native.QueryStreamAsync($"SELECT v FROM {table} LIMIT 1{Settings}"))
        {
