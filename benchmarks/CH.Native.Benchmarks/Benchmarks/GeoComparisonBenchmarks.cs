...
public class GeoComparisonBenchmarks
{
    ...
    public async Task<int> Native_SelectPoint()
    {
        ...
        await connection.OpenAsync();
        int count = 0;
        await foreach (var row in connection.QueryStreamAsync($"SELECT geom FROM {PointTable}"))
        {
    ...
    public async Task<int> Native_SelectRing()
    {
        ...
        await connection.OpenAsync();
        int count = 0;
        await foreach (var row in connection.QueryStreamAsync($"SELECT geom FROM {RingTable}"))
        {
    ...
    public async Task<int> Native_SelectMultiPolygon()
    {
        ...
        await connection.OpenAsync();
        int count = 0;
        await foreach (var row in connection.QueryStreamAsync($"SELECT geom FROM {MultiPolygonTable}"))
        {
            _ = (Point[][][])row.GetFieldValue<object>("geom");
            count++;
        }
    ...
    public async Task<int> Native_SelectGeometry()
    {
        ...
        await connection.OpenAsync();
        int count = 0;
        await foreach (var row in connection.QueryStreamAsync($"SELECT geom FROM {GeometryTable}"))
        {
