...
public class BulkInsertBoundaryTests
{
    ...
    public async Task BulkInsert_String_NullByte_RoundTrips()
    {
        ...
        try
        {
            ...
            await foreach (var row in connection.QueryStreamAsync($"SELECT Value FROM {tableName} WHERE Id = 1"))
            {
    ...
    public async Task BulkInsert_SingleRow_RoundTrips()
    {
        ...
        try
        {
            ...
            string readBack = null!;
            await foreach (var row in connection.QueryStreamAsync($"SELECT Name FROM {tableName} WHERE Id = 42"))
            {
