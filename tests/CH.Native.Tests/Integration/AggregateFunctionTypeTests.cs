...
public class AggregateFunctionTypeTests
{
    ...
    public async Task SelectStar_FromAggregatingMv_Succeeds_ForSumState()
    {
        ...
        try
        {
            ...
            await foreach (var r in conn.StreamAsync($"SELECT id, v_state FROM {mv} ORDER BY id"))
            {
    ...
    public async Task SelectStar_FromAggregatingMv_Succeeds_ForCountAndMinMaxStates()
    {
        ...
        try
        {
            ...
            await foreach (var r in conn.StreamAsync($"SELECT id, c_state, min_state, max_state FROM {mv} ORDER BY id"))
            {
                rows.Add((
