...
public class CancellationTests
{
    ...
    public async Task QueryAsync_EarlyBreak_ConnectionStillUsable()
    {
        ...
        int count = 0;
        await foreach (var row in connection.QueryStreamAsync("SELECT number FROM numbers(1000000)"))
        {
            count++;
            if (count >= 10)
    ...
    public async Task QueryAsync_WithCancellationToken_CancelsIteration()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(async () =>
        {
            ...
            await foreach (var row in connection.QueryStreamAsync("SELECT number FROM numbers(10000000000)", cts.Token))
            {
                count++;
            }
        });
