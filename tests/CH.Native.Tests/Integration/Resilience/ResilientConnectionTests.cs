...
public class ResilientConnectionTests
{
    ...
    public async Task QueryAsync_StreamsResults()
    {
        ...
        await foreach (var row in connection.QueryStreamAsync("SELECT number FROM system.numbers LIMIT 10"))
        {
    ...
    public async Task QueryAsyncT_MapsToObjects()
    {
        ...
        try
        {
            ...
            await foreach (var item in connection.QueryStreamAsync<TestRecord>($"SELECT id, name FROM {tableName} ORDER BY id"))
            {
                results.Add(item);
            }
    ...
    public async Task MultipleOperations_InSequence_Succeed()
    {
        ...
        try
        {
            ...
            // Stream query
            var sum = 0.0;
            await foreach (var row in connection.QueryStreamAsync($"SELECT value FROM {tableName}"))
            {
