...
public class AggregateFunctionSmokeTests
{
        ...
        string columnDef, string insertValues, string selectExpr, string orderBy)
    {
        ...
        try
        {
            ...
            var native = await NativeQueryHelper.QueryAsync(_fixture.NativeConnectionString, sql);
            var driver = await DriverQueryHelper.QueryAsync(_fixture.DriverConnectionString, sql);
