...
public class ComplexTypeMatrixTests
{
    ...
    public async Task Map_RoundTrips(string image)
    {
        ...
        try
        {
            ...
            await foreach (var r in conn.StreamAsync($"SELECT id, m FROM {table} ORDER BY id"))
            {
    ...
    public async Task Tuple_RoundTrips(string image)
    {
        ...
        try
        {
            ...
            await foreach (var r in conn.StreamAsync($"SELECT id, t.1, t.2, t.3 FROM {table} ORDER BY id"))
            {
                got.Add(new object?[]
                {
    ...
    public async Task IPv4_And_IPv6_RoundTrip(string image)
    {
        ...
        try
        {
            ...
            await foreach (var r in conn.StreamAsync<IpRow>($"SELECT id, v4, v6 FROM {table} ORDER BY id"))
                got.Add(r);
    ...
    public async Task Uuid_RoundTrips(string image)
    {
        ...
        try
        {
            ...
            await foreach (var r in conn.StreamAsync<UuidRow>($"SELECT id, u FROM {table} ORDER BY id"))
                got.Add(r);
