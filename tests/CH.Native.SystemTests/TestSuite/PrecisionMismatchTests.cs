...
public class PrecisionMismatchTests
{
    ...
    public async Task Decimal128_AboveClrMaxValue_ClampsSilentlyOnReadAsDecimal()
    {
        ...
        try
        {
            ...
            await foreach (var row in conn.QueryStreamAsync<NativeDecimalRow>($"SELECT id, v FROM {table}"))
                nativeValue = row.V;
            ...
            await foreach (var row in conn.QueryStreamAsync<NativeDecimalRow>($"SELECT id, v FROM {table} WHERE id = 2"))
                negNative = row.V;
    ...
    public async Task Decimal128_AtClrMaxValue_RoundTripsExactly()
    {
        ...
        try
        {
            ...
            DecimalRow? row = null;
            await foreach (var r in conn.QueryStreamAsync<DecimalRow>($"SELECT id, v FROM {table}"))
                row = r;
    ...
    public async Task Float64_NarrowingToFloat32_PrecisionIsLost_ButNoThrow()
    {
        ...
        try
        {
            ...
            await foreach (var r in conn.QueryStreamAsync<DoubleRow>($"SELECT id, v FROM {table}"))
                doubleRow = r;
            ...
            try
            {
                await foreach (var r in conn.QueryStreamAsync<FloatRow>($"SELECT id, v FROM {table}"))
                    floatRow = r;
            }
    ...
    public async Task UInt64_MaxValue_BulkInsertAndRead_RoundTrips()
    {
        ...
        try
        {
            ...
            await foreach (var r in conn.QueryStreamAsync<ULongRow>($"SELECT id, v FROM {table} ORDER BY id"))
                rows.Add(r);
