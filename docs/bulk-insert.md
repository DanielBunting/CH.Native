...
- **Failed initialization is retryable.** If the lazy (or explicit) initialization fails at a clean protocol boundary, the inserter is left un-initialized and the next add or `InitAsync` retries it on the same connection. That covers both *pre-wire* errors (connection busy, invalid `ColumnTypes`, a token already cancelled at entry — nothing was sent) and *server rejections of the INSERT statement* (missing table, permission denied, unknown column — the server rejects the query and replies with a clean exception envelope, so the wire returns to idle). The one exception is **cancellation after the INSERT is already in flight**: the cancel cannot cleanly drain an in-progress INSERT, so the *connection* is marked broken and retrying requires a fresh `ClickHouseConnection`. Even then the inserter is left un-latched — the next add reports the truthful "connection is broken" rather than a misleading "cancelled CompleteAsync" error.
- **A never-used inserter never contacts the server.** `CompleteAsync()` or dispose on an inserter that was never initialized *and* never had an add call is a silent no-op — no INSERT is sent, so the table is *not* validated. This keys off the *method call*, not the row count: `AddAsync`/`AddRangeAsync`/`AddRangeStreamingAsync` all initialize at entry, before iterating, so even an *empty* `AddRangeAsync(...)` opens the INSERT and claims the connection. If you need an empty insert to verify the table exists (as `connection.BulkInsertAsync` does for empty sources), call `InitAsync()` first.
...
- **Complete to persist and to observe errors.** Call `CompleteAsync` to flush, commit, and surface any worker failure (the first fault, or an `AggregateException` if several). Disposing without completing abandons un-flushed rows and does **not** report worker errors — always `CompleteAsync` if you need to know whether the insert succeeded.
- **`RowsWritten` is best-effort after a failure.** It equals the total inserted after a successful `CompleteAsync`; after a partial failure or a cancellation it is a lower bound (the workers that committed before the failure), reflecting the non-atomic nature of the fan-out.
